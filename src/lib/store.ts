...
export interface AppSettings {
  ...
  branchId?: string; // Branch ID (usually "01")
...
export interface ZRAInvoice {
  ...
  date: string;
  invoiceType: "Normal" | "Copy" | "Proforma" | "Training";
  tin: string;
...
export const generateZRAInvoice = (sale: Sale, invoiceType: "Normal" | "Copy" | "Proforma" | "Training" = "Normal"): ZRAInvoice | null => {
  const settings = getSettings();
  const taxRate = settings.taxRate || 16;
  
  if (!settings.tin) {
    return null; // TIN is required for ZRA invoices
  }
  ...
  return {
    ...
    invoiceType,
    tin: settings.tin || "",
    vatRegistration: settings.vatRegistration || "",
